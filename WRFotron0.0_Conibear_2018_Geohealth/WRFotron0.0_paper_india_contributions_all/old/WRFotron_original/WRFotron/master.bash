...
/bin/sed -f sedChemScript   < ${chainDir}/${emissInpFile}        > anthro_emis.inp
...
then
  if $isDependent
  then
    pretxt=$(${submitCommand} -w "ended(${depjobnr})" < pre.bash)
  else
    pretxt=$(${submitCommand} < pre.bash)
  fi
  prejobnr=$(echo $pretxt | grep -o "<[0-9\]\{1,10\}>" | sed -s "s/[<>]//g")

  maintxt=$(${submitCommand} -w "ended(${prejobnr})" < main.bash)
  mainjobnr=$(echo $maintxt | grep -o "<[0-9\]\{1,10\}>" | sed -s "s/[<>]//g")

  posttxt=$(${submitCommand} -w "ended(${mainjobnr})" < post.bash)
  postjobnr=$(echo $posttxt | grep -o "<[0-9\]\{1,10\}>" | sed -s "s/[<>]//g")
